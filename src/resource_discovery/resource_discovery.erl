...
%%  获取资源
fetch_resource(Key) ->
  gen_server:call(?SERVER, {fetch_resource, Key}).


handle_call({fetch_resource, Key}, _From, State) ->
  {reply, dict:fetch(Key, State#state.found_resource), State}.
...
handle_cast({add_local_resource, {Key, Value}}, State) ->
  ...
  NewLocalResource = dict:store(Key, Value, OldLocalResource),
...
%%  更新线程状态
  {noreply, State#state{found_resource = dict:from_list(NewFoundResource)}}.
